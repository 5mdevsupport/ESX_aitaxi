...
RegisterNetEvent('esx_aiTaxi:Notif')
...
AddEventHandler('esx_aiTaxi:callTaxi', function()
	local playerPed = PlayerPedId()
...
AddEventHandler('esx_aiTaxi:cancelTaxi', function()
	local playerPed = PlayerPedId()
	...
	if DoesEntityExist(taxiVeh) then 
		if not IsPedInAnyVehicle(playerPed, false) or not IsPedInAnyTaxi(playerPed) then
			...
			DisplayNotify("Fout", "Je kan de opdracht niet annuleren omdat je al in de Taxi zit.")
		end
	else
		DisplayNotify("Fout", "Je kan geen taxi annuleren als je er nog geen besteld hebt.")
	end
end)
